...
다만, 세 번째 테스트에서는 *'라파엘'*이라는 글자만 얻고 했지만 문장 패턴이 '내 이름은 ~야'로 지정되어 있기 때문에 *'라파엘이'* 라는 정보를 가져오게 됩니다. 이와 같이 sys.any는 모든 말이 담기기 때문에 예문에 특정 패턴으로 지정해서 사용 시에는 주의해서 사용해야 합니다. 
